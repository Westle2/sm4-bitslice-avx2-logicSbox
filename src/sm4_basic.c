/*=========================================================
  File: src/sm4_basic.c
  Description:
    Standard SM4 reference implementation (basic version)
    - 32-round encryption
    - official S-box
    - key schedule
    - ECB single-block encrypt core

  Build:
    gcc -O3 sm4_basic.c -c

=========================================================*/

#include <stdint.h>
#include <string.h>

/* ==================== SM4 Constants ==================== */

static const uint32_t FK[4] = {
    0xA3B1BAC6, 0x56AA3350,
    0x677D9197, 0xB27022DC
};

static const uint32_t CK[32] = {
    0x00070E15,0x1C232A31,0x383F464D,0x545B6269,
    0x70777E85,0x8C939AA1,0xA8AFB6BD,0xC4CBD2D9,
    0xE0E7EEF5,0xFC030A11,0x181F262D,0x343B4249,
    0x50575E65,0x6C737A81,0x888F969D,0xA4ABB2B9,
    0xC0C7CED5,0xDCE3EAF1,0xF8FF060D,0x141B2229,
    0x30373E45,0x4C535A61,0x686F767D,0x848B9299,
    0xA0A7AEB5,0xBCC3CAD1,0xD8DFE6ED,0xF4FB0209,
    0x10171E25,0x2C333A41,0x484F565D,0x646B7279
};

static const uint8_t SBOX[256] = {

};

/* ==================== Utility ==================== */

static uint32_t rotl32(uint32_t x, int n)
{
    return (x << n) | (x >> (32 - n));
}

static uint32_t load_be32(const uint8_t *b)
{
    return ((uint32_t)b[0] << 24) |
           ((uint32_t)b[1] << 16) |
           ((uint32_t)b[2] <<  8) |
           ((uint32_t)b[3]);
}

static void store_be32(uint32_t x, uint8_t *b)
{
    b[0] = (x >> 24) & 0xFF;
    b[1] = (x >> 16) & 0xFF;
    b[2] = (x >>  8) & 0xFF;
    b[3] = x & 0xFF;
}

/* ==================== Core Transform ==================== */

static uint32_t tau(uint32_t a)
{
    uint8_t b0 = SBOX[(a >> 24) & 0xFF];
    uint8_t b1 = SBOX[(a >> 16) & 0xFF];
    uint8_t b2 = SBOX[(a >>  8) & 0xFF];
    uint8_t b3 = SBOX[a & 0xFF];

    return ((uint32_t)b0 << 24) |
           ((uint32_t)b1 << 16) |
           ((uint32_t)b2 <<  8) |
           ((uint32_t)b3);
}

static uint32_t L(uint32_t b)
{
    return b ^
           rotl32(b, 2) ^
           rotl32(b,10) ^
           rotl32(b,18) ^
           rotl32(b,24);
}

static uint32_t Lp(uint32_t b)
{
    return b ^
           rotl32(b,13) ^
           rotl32(b,23);
}

static uint32_t T(uint32_t x)
{
    return L(tau(x));
}

static uint32_t Tp(uint32_t x)
{
    return Lp(tau(x));
}

/* ==================== Key Schedule ==================== */

void sm4_setkey_basic(const uint8_t key[16], uint32_t rk[32])
{
    uint32_t MK[4];
    uint32_t K[36];

    for(int i=0;i<4;i++)
        MK[i] = load_be32(key + i*4);

    for(int i=0;i<4;i++)
        K[i] = MK[i] ^ FK[i];

    for(int i=0;i<32;i++)
    {
        K[i+4] = K[i] ^ Tp(K[i+1]^K[i+2]^K[i+3]^CK[i]);
        rk[i] = K[i+4];
    }
}

/* ==================== Encrypt One Block ==================== */

void sm4_encrypt_block_basic(
    const uint8_t in[16],
    uint8_t out[16],
    const uint32_t rk[32])
{
    uint32_t X[36];

    for(int i=0;i<4;i++)
        X[i] = load_be32(in + i*4);

    for(int i=0;i<32;i++)
    {
        X[i+4] = X[i] ^
                 T(X[i+1]^X[i+2]^X[i+3]^rk[i]);
    }

    store_be32(X[35], out + 0);
    store_be32(X[34], out + 4);
    store_be32(X[33], out + 8);
    store_be32(X[32], out +12);
}

/* ==================== ECB Multi-block ==================== */

void sm4_encrypt_ecb_basic(
    const uint8_t *in,
    uint8_t *out,
    int blocks,
    const uint32_t rk[32])
{
    for(int i=0;i<blocks;i++)
    {
        sm4_encrypt_block_basic(
            in  + i*16,
            out + i*16,
            rk
        );
    }
}
